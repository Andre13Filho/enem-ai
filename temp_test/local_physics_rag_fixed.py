...
class LocalPhysicsRAG:
    ...
    def _download_file(self, url: str, local_path: str):
        """Baixa um arquivo de uma URL para um caminho local com tratamento robusto de erros."""
        max_retries = 3
        retry_count = 0
        
        while retry_count < max_retries:
            try:
                print(f"📥 Tentativa {retry_count + 1} de baixar {url}")
                        ...
                        for chunk in r.iter_content(chunk_size=8192):
                            if chunk:
                                ...
                                if total_size > 0:
                                    ...
                                    print(f"\rBaixando: {percent:.1f}% de {total_size/1024/1024:.1f}MB", end="")
                
                print(f"\n✅ Arquivo baixado com sucesso: {local_path}")
                
                # Verifica se o arquivo foi baixado corretamente
                if os.path.exists(local_path) and os.path.getsize(local_path) > 0:
                    return True
                else:
                    print(f"⚠️ Arquivo baixado com tamanho zero: {local_path}")
                    retry_count += 1
        ...
        print(f"❌ Falha após {max_retries} tentativas de baixar {url}")
        return False
    ...
    def _ensure_faiss_index_is_ready(self) -> bool:
        ...
        # URLs dos arquivos no Hugging Face - URLs atualizados
        ...
        print(f"📥 Baixando índice FAISS de: {faiss_url}")
        print(f"📥 Baixando índice PKL de: {pkl_url}")
