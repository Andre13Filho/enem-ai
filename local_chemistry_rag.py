#!/usr/bin/env python3
"""
Sistema RAG Local para Professora Luciana - Química
...
FAISS_INDEX_DIR = "faiss_index_chemistry"
...
class LocalChemistryRAG:
    """Sistema RAG que carrega um índice FAISS remoto para Química."""
    ...
    def __init__(self):
        ...
        self.chemistry_folder_path = FAISS_INDEX_DIR
    ...
    def _ensure_faiss_index_is_ready(self) -> bool:
        ...
        index_file = os.path.join(FAISS_INDEX_DIR, "index_chemistry.faiss")
        pkl_file = os.path.join(FAISS_INDEX_DIR, "index_chemistry.pkl")

        # Verifica se os dois arquivos já existem
        if os.path.exists(index_file) and os.path.exists(pkl_file):
            print("✅ Índice FAISS de química já existe localmente.")
            return True

        st.info("📥 Baixando índice de química do Hugging Face...")
        print("📥 Baixando índice de química do Hugging Face...")

        # URLs dos arquivos no Hugging Face
        faiss_url = "https://huggingface.co/Andre13Filho/rag_enem/resolve/main/index_chemistry.faiss"
        pkl_url = "https://huggingface.co/Andre13Filho/rag_enem/resolve/main/index_chemistry.pkl"
        ...
        if faiss_success and pkl_success:
            st.success("✅ Índice de química baixado com sucesso!")
            return True
        else:
            st.error("❌ Falha ao baixar os arquivos do índice de química.")
    ...
    def initialize(self, api_key: str) -> bool:
        ...
        try:
            st.info("📚 Carregando base de conhecimento de química (FAISS)...")
            print("📚 Carregando base de conhecimento de química (FAISS)...")
        ...
        try:
            ...
            # Adiciona o prompt personalizado para Química
            prompt_template = """Você é a Professora Luciana, especialista em química do ENEM. Responda como uma professora para uma estudante de 17 anos chamada Sther.

🔥 REGRAS DE FORMATAÇÃO QUÍMICA (CRÍTICO - SEMPRE SEGUIR):

1. **DELIMITADORES OBRIGATÓRIOS:**
   - Fórmulas no meio do texto: $sua-formula-aqui$
   - Fórmulas em destaque: $$sua-formula-aqui$$
   - NUNCA use \\text{concentração} sozinho - sempre use $\\text{concentração}$

2. **EXEMPLOS CORRETOS:**
   ✅ A concentração é calculada por $C = \\frac{n}{V}$
   ✅ O pH é definido como: $$pH = -\\log[H^+]$$
   ✅ Para equações químicas: $$2H_2 + O_2 \\rightarrow 2H_2O$$

3. **COMANDOS LATEX ESSENCIAIS:**
   - Frações: $\\frac{numerador}{denominador}$
   - Raízes: $\\sqrt{x}$ ou $\\sqrt[n]{x}$
   - Texto em fórmulas: $\\text{concentração} = \\frac{\\text{mols}}{\\text{volume}}$
   - Potências: $H^+$, $e^{-}$
   - Índices: $H_2$, $O_2$
...
5. **ESTILO DA PROFESSORA LUCIANA:**
   - Use analogias das séries que a Sther gosta (FRIENDS, Big Bang Theory, etc.)
   - Seja didática e paciente
   - Conecte conceitos químicos com exemplos práticos
   - Explique como Monica organizaria os elementos na tabela periódica

Com base no CONTEXTO abaixo, responda à PERGUNTA do aluno.
Se a resposta não estiver no contexto, use seu conhecimento em química, mas mantenha o estilo.
...
RESPOSTA (com fórmulas bem formatadas e estilo da Professora Luciana):
"""
...
def get_local_chemistry_rag_instance():
    """
    Retorna uma instância única (singleton) do LocalChemistryRAG.
    Isso evita a inicialização no momento da importação.
    """
    global _singleton_instance
    if _singleton_instance is None:
        _singleton_instance = LocalChemistryRAG()
