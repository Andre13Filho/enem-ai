#!/usr/bin/env python3
"""
Sistema RAG Local para Professor Letícia - Língua Portuguesa
...
FAISS_INDEX_DIR = "faiss_index_portuguese"
...
class LocalPortugueseRAG:
    ...
    def __init__(self):
        ...
        self.portuguese_folder_path = FAISS_INDEX_DIR
    ...
    def _ensure_faiss_index_is_ready(self) -> bool:
        ...
        index_file = os.path.join(FAISS_INDEX_DIR, "index_portuguese.faiss")
        pkl_file = os.path.join(FAISS_INDEX_DIR, "index_portuguese.pkl")
        ...
        st.info("📥 Baixando índice de português do Hugging Face...")
        print("📥 Baixando índice de português do Hugging Face...")

        # URLs dos arquivos no Hugging Face
        faiss_url = "https://huggingface.co/Andre13Filho/rag_enem/resolve/main/index_portuguese.faiss"
        pkl_url = "https://huggingface.co/Andre13Filho/rag_enem/resolve/main/index_portuguese.pkl"
        ...
        if faiss_success and pkl_success:
            st.success("✅ Índice de português baixado com sucesso!")
            return True
        else:
            st.error("❌ Falha ao baixar os arquivos do índice de português.")
    ...
    def initialize(self, api_key: str) -> bool:
        ...
        try:
            st.info("📚 Carregando base de conhecimento de português (FAISS)...")
            print("📚 Carregando base de conhecimento de português (FAISS)...")
        ...
        try:
            ...
            # Adiciona o prompt personalizado para a Professora Letícia
            prompt_template = """Você é a Professora Letícia, especialista em Língua Portuguesa e Literatura do ENEM. Responda como uma professora para uma estudante de 17 anos chamada Sther.

🎬 INICIE SEMPRE com analogia das séries da Sther (FRIENDS, Big Bang Theory, Stranger Things, Grey's Anatomy, WandaVision)
👋 Cumprimento: "Olá Sther!"
📚 Explicação DIRETA do português/literatura
📝 Exemplo prático quando relevante
🎯 Conecte de volta com a analogia
❓ Termine perguntando sobre exercícios
...
Se a resposta não estiver no contexto, use seu conhecimento em português e literatura, mas mantenha o estilo.
...
RESPOSTA (seguindo o formato da Professora Letícia):
"""
...
def get_local_portuguese_rag_instance():
    """
    Retorna uma instância única (singleton) do LocalPortugueseRAG.
