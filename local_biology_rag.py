#!/usr/bin/env python3
"""
Sistema RAG Local para Professor Roberto - Biologia
...
FAISS_INDEX_DIR = "faiss_index_biology"
...
class LocalBiologyRAG:
    """Sistema RAG que carrega um índice FAISS remoto para Biologia."""
    ...
    def __init__(self):
        ...
        self.biology_folder_path = FAISS_INDEX_DIR
    ...
    def _ensure_faiss_index_is_ready(self) -> bool:
        ...
        index_file = os.path.join(FAISS_INDEX_DIR, "index_biology.faiss")
        pkl_file = os.path.join(FAISS_INDEX_DIR, "index_biology.pkl")

        # Verifica se os dois arquivos já existem
        if os.path.exists(index_file) and os.path.exists(pkl_file):
            print("✅ Índice FAISS de biologia já existe localmente.")
            return True

        st.info("📥 Baixando índice de biologia do Hugging Face...")
        print("📥 Baixando índice de biologia do Hugging Face...")

        # URLs dos arquivos no Hugging Face
        faiss_url = "https://huggingface.co/Andre13Filho/rag_enem/resolve/main/index_biology.faiss"
        pkl_url = "https://huggingface.co/Andre13Filho/rag_enem/resolve/main/index_biology.pkl"
        ...
        if faiss_success and pkl_success:
            st.success("✅ Índice de biologia baixado com sucesso!")
            return True
        else:
            st.error("❌ Falha ao baixar os arquivos do índice de biologia.")
    ...
    def initialize(self, api_key: str) -> bool:
        ...
        try:
            st.info("📚 Carregando base de conhecimento de biologia (FAISS)...")
            print("📚 Carregando base de conhecimento de biologia (FAISS)...")
        ...
        try:
            ...
            # Adiciona o prompt personalizado para Biologia
            prompt_template = """Você é o Professor Roberto, especialista em biologia do ENEM. Responda como um professor para uma estudante de 17 anos chamada Sther.

🔥 REGRAS DE FORMATAÇÃO BIOLÓGICA (CRÍTICO - SEMPRE SEGUIR):

1. **DELIMITADORES OBRIGATÓRIOS:**
   - Fórmulas no meio do texto: $sua-formula-aqui$
   - Fórmulas em destaque: $$sua-formula-aqui$$
   - NUNCA use \\text{célula} sozinho - sempre use $\\text{célula}$

2. **EXEMPLOS CORRETOS:**
   ✅ A fotossíntese: $$6CO_2 + 6H_2O \\rightarrow C_6H_{12}O_6 + 6O_2$$
   ✅ A respiração celular: $$C_6H_{12}O_6 + 6O_2 \\rightarrow 6CO_2 + 6H_2O + ATP$$
   ✅ Para DNA: $$A-T, G-C$$

3. **COMANDOS LATEX ESSENCIAIS:**
   - Frações: $\\frac{numerador}{denominador}$
   - Raízes: $\\sqrt{x}$ ou $\\sqrt[n]{x}$
   - Texto em fórmulas: $\\text{célula} = \\text{unidade básica da vida}$
   - Potências: $ATP$, $CO_2$
   - Índices: $H_2O$, $C_6H_{12}O_6$
...
5. **ESTILO DO PROFESSOR ROBERTO:**
   - Use analogias das séries que a Sther gosta (FRIENDS, Big Bang Theory, etc.)
   - Seja didático e paciente
   - Conecte conceitos biológicos com exemplos práticos
   - Explique como Monica organizaria as organelas da célula
...
Se a resposta não estiver no contexto, use seu conhecimento em biologia, mas mantenha o estilo.
...
RESPOSTA (com fórmulas bem formatadas e estilo do Professor Roberto):
"""
...
def get_local_biology_rag_instance():
    """
    Retorna uma instância única (singleton) do LocalBiologyRAG.
    Isso evita a inicialização no momento da importação.
    """
    global _singleton_instance
    if _singleton_instance is None:
        _singleton_instance = LocalBiologyRAG()
